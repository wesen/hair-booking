...
// API: stylistApi.getStylistMe() → StylistMeDto; stylistApi.getServices()
...
interface YouPageProps {
  me: StylistMeDto | null | undefined;
  rating: string;
  reviewCount: string;
  weekEarnings: string;
  rebookRate: string;
...
const NAV_GROUPS: NavGroup[] = [
  {
    ...
    items: [
      ...
      { key: "Payouts",            value: "$4,820 pending",    arrow: true },
    ],
  },
  {
    group: "Your page",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)" }}>
          {[
            ...
            <div key={s.k} style={{
              ...
              <div style={{ fontFamily: font.serif, fontStyle: "italic", fontSize: 12, color: color.soft, marginTop: 2 }}>
                {s.sub}
              </div>
            </div>
          ))}
        ...
        <div key={g.group} style={{ padding: "0 22px 14px" }}>
          ...
          {g.items.map((it) => (
            <NavRow
              key={it.key}
              it={it}
              onSignOut={it.key === "Sign out" ? onSignOut : undefined}
            />
          ))}
        </div>
      ))}
...
function NavRow({ it, onSignOut }: { it: NavItem; onSignOut?: () => void }) {
  return (
    <div
      style={{
        padding: "14px 0",
        ...
        cursor: it.danger ? "pointer" : "default",
      }}
      onClick={onSignOut}
    >
      <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {it.value && (
          <div style={{
            ...
            fontStyle: "italic",
            fontSize: 14,
            color: it.key === "Payouts" ? color.coral : color.soft,
          }}>
            {it.value}
          </div>
        )}
